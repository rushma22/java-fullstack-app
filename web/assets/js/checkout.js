    ...
    if (response.ok) { //200
        ...
        if (json.status) {
            ...
            city_select.addEventListener("change", (e) => {
                ...
                if (deliveryTypes.length >= 2) {
                    ...
                    st_order_shipping_tr.querySelector("#st-product-shipping-charges")
                            .innerHTML = new Intl.NumberFormat("en-US", {minimumFractionDigits: 2}).format(shipping_charges);
                    st_tbody.appendChild(st_order_shipping_tr);

                    st_order_total_tr.querySelector("#st-order-total-amount")
                            .innerHTML = new Intl.NumberFormat("en-US", {minimumFractionDigits: 2}).format(shipping_charges + total);
                    st_tbody.appendChild(st_order_total_tr);
                } else {
                    console.warn("Delivery types not available. Skipping shipping calculation.");
                }
            ...
            if (json.message === "empty-cart") {
                ...
                console.log("tempty car", json.message)
            }
        }
    } else {
